...
use pdcan_protocol::{BOARD_TARGET, Header, MESSAGE_DEFINITIONS, MessageClass, TargetKind};
...
fn render_dbc() -> Result<String, String> {
    ...
    output.push_str(
        "CM_ \"Generated from pdcan_protocol::MESSAGE_DEFINITIONS. IDs use node 1, port 0, and requester 15 as representative values; those fields are dynamic on the bus.\";\n\n",
    );
    ...
    for definition in MESSAGE_DEFINITIONS {
        ...
        let dbc_id = header.encode().get() | 0x8000_0000;
        let transmitter = if definition.class == MessageClass::Control {
            "Host"
        } else {
            "Backpack"
        };
        ...
        if definition.requester_scoped && definition.payload_len >= 8 {
            output.push_str(
