...
async fn main(spawner: embassy_executor::Spawner) -> ! {
    ...
    // The finalized board uses active-high supply PWM. Start both 3-wire fan
    // channels at the safe full-on state before initializing the wider system.
    let _fan_1 = Output::new(peripherals.PA2, Level::High, Speed::Low);
    let _fan_2 = Output::new(peripherals.PA0, Level::High, Speed::Low);
    let _status_led = Output::new(peripherals.PA1, Level::Low, Speed::VeryHigh);
